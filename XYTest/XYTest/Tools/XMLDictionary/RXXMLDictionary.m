...
@interface RXXMLDictionaryParser () <NSXMLParserDelegate>
...
@implementation RXXMLDictionaryParser

+ (RXXMLDictionaryParser *)sharedInstance
{
    static dispatch_once_t once;
    static RXXMLDictionaryParser *sharedInstance;
    dispatch_once(&once, ^{
        
        sharedInstance = [[RXXMLDictionaryParser alloc] init];
    });
...
{
    RXXMLDictionaryParser *copy = [[[self class] allocWithZone:zone] init];
...
{	
    ...
    {
        NSDictionary *attributes = [(NSDictionary *)node rx_attributes];
        NSMutableString *attributeString = [NSMutableString string];
        for (NSString *key in [attributes allKeys])
        {
            [attributeString appendFormat:@" %@=\"%@\"", [[key description] rx_XMLEncodedString], [[attributes[key] description] rx_XMLEncodedString]];
        }
        
        NSString *innerXML = [node rx_innerXML];
        if ([innerXML length])
        {
    ...
    {
        return [NSString stringWithFormat:@"<%1$@>%2$@</%1$@>", nodeName, [[node description] rx_XMLEncodedString]];
    }
}
...
{	
	...
	if (!top.rx_attributes && !top.rx_childNodes && !top.rx_comments)
    {
        ...
        if (nodeName)
        {
            ...
            if (top.rx_innerText && _collapseTextNodes)
            {
                if ([parentNode isKindOfClass:[NSArray class]])
                {
                    parentNode[[parentNode count] - 1] = top.rx_innerText;
                }
                else
                {
                    newTop[nodeName] = top.rx_innerText;
                }
            }
            else if (!top.rx_innerText && _stripEmptyNodes)
            {
                if ([parentNode isKindOfClass:[NSArray class]])
                {
            ...
            else if (!top.rx_innerText && !_collapseTextNodes && !_stripEmptyNodes)
            {
...
@implementation NSDictionary(RXXMLDictionary)

+ (NSDictionary *)rx_dictionaryWithXMLParser:(NSXMLParser *)parser
{
	return [[[RXXMLDictionaryParser sharedInstance] copy] dictionaryWithParser:parser];
}

+ (NSDictionary *)rx_dictionaryWithXMLData:(NSData *)data
{
	return [[[RXXMLDictionaryParser sharedInstance] copy] dictionaryWithData:data];
}

+ (NSDictionary *)rx_dictionaryWithXMLString:(NSString *)string
{
	return [[[RXXMLDictionaryParser sharedInstance] copy] dictionaryWithString:string];
}

+ (NSDictionary *)rx_dictionaryWithXMLFile:(NSString *)path
{
	return [[[RXXMLDictionaryParser sharedInstance] copy] dictionaryWithFile:path];
}

- (NSDictionary *)rx_attributes
{
...
- (NSString *)rx_innerXML
{	
	NSMutableArray *nodes = [NSMutableArray array];
	
	for (NSString *comment in [self rx_comments])
	{
        [nodes addObject:[NSString stringWithFormat:@"<!--%@-->", [comment rx_XMLEncodedString]]];
	}
    
    NSDictionary *childNodes = [self rx_childNodes];
	for (NSString *key in childNodes)
	{
		[nodes addObject:[RXXMLDictionaryParser XMLStringForNode:childNodes[key] withNodeName:key]];
	}
	
    NSString *text = [self rx_innerText];
    if (text)
    {
        [nodes addObject:[text rx_XMLEncodedString]];
    }
	
	return [nodes componentsJoinedByString:@"\n"];
}

- (NSString *)rx_XMLString
{
    if ([self count] == 1 && ![self rx_nodeName])
    {
        //ignore outermost dictionary
        return [self rx_innerXML];
    }
    else
    {
        return [RXXMLDictionaryParser XMLStringForNode:self withNodeName:[self rx_nodeName] ?: @"root"];
    }
}

- (NSArray *)rx_arrayValueForKeyPath:(NSString *)keyPath
{
...
- (NSString *)rx_stringValueForKeyPath:(NSString *)keyPath
{
    id value = [self valueForKeyPath:keyPath];
    ...
    {
        return [(NSDictionary *)value rx_innerText];
    }
    return value;
}

- (NSDictionary *)rx_dictionaryValueForKeyPath:(NSString *)keyPath
{
    id value = [self valueForKeyPath:keyPath];
    if ([value isKindOfClass:[NSArray class]])
    {
...
@implementation NSString (RXXMLDictionary)

- (NSString *)rx_XMLEncodedString
{	
